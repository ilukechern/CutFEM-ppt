...
\setmonofont{JuliaMono} %西文默认的等宽字体
...
%\begin{frame}{PrePoMax前后处理程序}
%    With the program, we can solve linear and non-linear (geometry, material, contact) \textbf{2D} and \textbf{3D static analyses}, \textbf{buckling} and \textbf{vibration analyses} or basic \textbf{thermal analyses}. \\
%    The program has a geometry importer and an in-built \textcolor{red}{NetGen} mesher, which can be used to create shell and solid meshes (tetrahedral only); a hexahedral mesh can be imported into the program (e.g. generated in \textcolor{red}{Salome} or \textcolor{red}{Gmsh}).
%\end{frame}
